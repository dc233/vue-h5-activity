<template>
  <div class="content">
    <!-- <van-nav-bar title="内景照片" left-arrow @click-left="onClickLeft">
    ...
    <div class="contentbody">
      <strong>内景照片：</strong>
      <span>
        要求照片清晰可见，具备餐饮 相关环境包括但不限于桌子，餐具等
      </span>
...
export default {
  data() {
    return {
      fileList: [],
      images: [require("../../../assets/img/interior.jpg")]
    };
  },
  computed: {},
  created() {
  ...
  methods: {
    ...
    async uploadimg(formData) {
      ...
      if (!!data.status && data.code === 1) {
        this.$toast("图片上传成功");
        this.setzfbinterior(data.data.src);
      } else {
        this.$toast(data.msg);
      }
    },
