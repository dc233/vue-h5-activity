<template>
  <div class="content">
    <!-- <van-nav-bar title="营业执照照片" left-arrow @click-left="onClickLeft">
    ...
    <div class="contentbody">
      <div><strong>营业执照照片：</strong></div>
      <div>
        <span>
          店内拍摄的门店营业执照，要求营业执照文本信息清晰可见。
        </span>
      </div>
...
export default {
  data() {
    return {
      fileList: [],
      images: [require("../../../assets/img/license.png")]
    };
  },
  computed: {},
  created() {
  ...
  methods: {
    ...
    async uploadimg(formData) {
      ...
      if (!!data.status && data.code === 1) {
        this.$toast("图片上传成功");
        this.setzfblicenese(data.data.src);
      } else {
        this.$toast(data.msg);
      }
    },
