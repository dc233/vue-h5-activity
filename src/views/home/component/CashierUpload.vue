<template>
  <div class="content">
    <!-- <van-nav-bar title="收银台照片" left-arrow @click-left="onClickLeft">
    ...
    <div class="contentbody">
      ...
      <div>
        <span>
          2.扫码支付场景需要展示具有微信支付logo的二维码物料
        </span>
      </div>
      <div>
        <span>
          3.刷卡支付场景需要展示具有微信支付logo的扫 码机具或扫描枪
        </span>
      </div>
...
export default {
  data() {
    return {
      fileList: [],
      images: [require("../../../assets/img/cashier.jpg")]
    };
  },
  computed: {},
  created() {
  ...
  methods: {
    ...
    async uploadimg(formData) {
      ...
      if (!!data.status && data.code === 1) {
        this.$toast("图片上传成功");
        this.stewxcashier(data.data.src);
      } else {
        this.$toast(data.msg);
      }
    },
